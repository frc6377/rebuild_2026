...
import static frc.robot.Constants.FuelScorerConstants.*;
...
public class IndexerSubsystem extends SubsystemBase {
    ...
    public static final double kCollectorSpeed = 0.5; // temporary
    ...
    public Command runCollector(Supplier<Double> percent) {
        return runEnd(
            () -> collectorBelt.set(Math.abs(percent.get()) * kCollectorSpeed),
            () -> collectorBelt.stopMotor(),
            this
        );
    }
    ...
    public Command runFeeder(Supplier<Double> percent) {
        return runEnd(
            () -> feederBelt.set(Math.abs(percent.get()) * kFeederSpeed),
            () -> feederBelt.stopMotor(),
            this
        );
    }
    ...
    public void periodic() {
        ...
        Logger.recordOutput("Collector Velocity (RPS)", collectorBelt.getVelocity().getValueAsDouble());
